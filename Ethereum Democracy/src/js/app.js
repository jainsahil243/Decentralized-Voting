    ...
    VotingContract.deployed().then(function(instance){
      ...
      instance.getNumOfCandidates().then(function(numOfCandidates){
        ...
        if (numOfCandidates == 0){
          ...
          instance.addCandidate("Rahul Gandhi - Congress","Congress").then(function(result){ 
            $("#candidate-box").append(`<div class='form-check'><input class='form-check-input' type='radio' name ="rad1" value='' id=${result.logs[0].args.candidateID}><label class='form-check-label' for=0>Rahul Gandhi - Congress</label></div>`)
          })
          instance.addCandidate("Narendra Modi - BJP","BJP").then(function(result){
            $("#candidate-box").append(`<div class='form-check'><input class='form-check-input' type='radio' name = "rad2" value='' id=${result.logs[0].args.candidateID}><label class='form-check-label' for=1>Narendra Modi - BJP</label></div>`)
          })
          ...
          for (var i = 0; i < numOfCandidates; i++ ){
            ...
            instance.getCandidate(i).then(function(data){
              $("#candidate-box").append(`<div class="form-check"><input class="form-check-input" type="radio" name="radio1" value="" id=${data[0]}><label class="form-check-label" for=${data[0]}>${window.web3.toAscii(data[1])}</label></div>`)
            })
          }
        }
  ...
  vote: function() {
    ...
    VotingContract.deployed().then(function(instance){
      instance.vote(uid,parseInt(candidateID)).then(function(result){
        //$("#msg").html("<p>Voted</p>")
        $("#msg").html("<p>Voted</p><script type='text/javascript'>function pageRedirect() {window.location.replace('http://localhost:4000/home');}setTimeout('pageRedirect()', 10000);</script>")
        //return instance.redirect('/Desktop/cryptoproject/views/home.hbs');
      })
    }).catch(function(err){ 
      console.error("ERROR! " + err.message)
    })
